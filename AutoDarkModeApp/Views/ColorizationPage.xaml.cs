...
public sealed partial class ColorizationPage : Page
{
    ...
    {
        if (ViewModel.AccentColorMode == ColorizationViewModel.ThemeColorMode.Manual)
        {
            var targetColor =
                ViewModel.SelectColorThemeMode == ApplicationTheme.Light
                    ? ViewModel.GetAdmConfig().ColorizationSwitch.Component.LightHex.ToColor()
                    : ViewModel.GetAdmConfig().ColorizationSwitch.Component.DarkHex.ToColor();

            int index = Array.FindIndex(windowsColors, color => color == targetColor);
            if (index >= 0)
            {
                WindowsColorGridView.SelectedIndex = index;
            }
            else
            {
    ...
    {
        foreach (var color in windowsColors)
        {
    ...
    private readonly Color[] windowsColors =
    [
