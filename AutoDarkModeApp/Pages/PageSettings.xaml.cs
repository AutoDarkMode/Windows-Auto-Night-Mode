...
public partial class PageSettings : Page
{
    ...
    private readonly string BetaVersionQueryURL =
        @"https://raw.githubusercontent.com/AutoDarkMode/AutoDarkModeVersion/master/version-beta.yaml";

    private readonly AdmConfigBuilder builder = AdmConfigBuilder.Instance();
    private readonly string curLanguage = Settings.Default.Language;
    ...
    public PageSettings()
    {
        ...
        try
        {
            ...
            var queryString = "SELECT * FROM RegistryValueChangeEvent WHERE Hive = 'HKEY_USERS' AND KeyPath = " +
                              $"'{sidString}\\\\" +
            ...
            var autostartWatcher = new ManagementEventWatcher(query);
            autostartWatcher.EventArrived += HandleAutostartEnabledEvent;
    ...
    private void UiHandler()
    {
        ...
        ComboBoxLanguageSelection.SelectedValue = Settings.Default.Language.Replace("-", "_");
        if (ComboBoxLanguageSelection.SelectedValue == null) ComboBoxLanguageSelection.SelectedValue = "en";
        ...
        if (!builder.Config.Tunable.Debug) CheckBoxTraceMode.Visibility = Visibility.Collapsed;
        ...
        if (builder.UpdaterData.LastCheck.Year.ToString().Equals("1"))
            TextBlockUpdateInfo.Text =
                $"{AdmProperties.Resources.UpdatesTextBlockLastChecked}: {AdmProperties.Resources.UpdatesTextBlockLastCheckedNever}";
        else
            TextBlockUpdateInfo.Text =
                $"{AdmProperties.Resources.UpdatesTextBlockLastChecked}: {builder.UpdaterData.LastCheck}";
    ...
    private async Task GetAutostartInfo(bool noToggle = false, bool toggleVisibility = true)
    {
        ...
        try
        {
            ...
            var autostartResponse =
                ApiResponse.FromString(
                    await MessageHandler.Client.SendMessageAndGetReplyAsync(Command.GetAutostartState));
            if (autostartResponse.StatusCode == StatusCode.Err)
            {
                ErrorMessageBoxes.ShowErrorMessageFromApi(autostartResponse, new AutoStartStatusGetException(),
    ...
    private void ComboBoxLanguageSelection_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (!init)
        {
            var selectedLanguage = ComboBoxLanguageSelection.SelectedValue.ToString().Replace("_", "-");
    ...
    {
        var error = AdmProperties.Resources.errorThemeApply + $"\n\nError ocurred in: {location}" + ex.Source + "\n\n" +
                    ex.Message;
    ...
    private async void CheckBoxLogonTask_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                result = ApiResponse.FromString(
                    await MessageHandler.Client.SendMessageAndGetReplyAsync(Command.AddAutostart));
                ...
                if (result.StatusCode != StatusCode.Ok)
                {
                    ...
                    builder.Config.Tunable.UseLogonTask = CheckBoxLogonTask.IsChecked ?? false;
                    builder.Save();
                    throw new AddAutoStartException("error while processing CheckBoxLogonTask",
                        "AutoDarkModeSvc.MessageParser.AddAutostart");
                }
            }
        }
        catch (Exception ex)
    ...
    private void CheckBoxHideTrayIcon_Click(object sender, RoutedEventArgs e)
    {
        if ((sender as CheckBox).IsChecked.Value)
        {
            MsgBox confirm = new(AdmProperties.Resources.SettingsPageTrayDisableMessageBoxContent,
                AdmProperties.Resources.SettingsPageTrayDisableMessageBoxHeader, "info", "yesno")
            {
                Owner = Window.GetWindow(this)
            };
            var result = confirm.ShowDialog() ?? false;
            if (!result)
            {
    ...
    {
        var offerDowngrade = false;
        if ((sender as RadioButton).IsChecked.Value)
        {
            if (builder.Config.Updater.VersionQueryUrl != null) offerDowngrade = true;
        ...
        try
        {
            ...
            if (offerDowngrade)
            {
                _ = ApiResponse.FromString(
                    await MessageHandler.Client.SendMessageAndGetReplyAsync(Command.CheckForUpdate));
                var response =
                    ApiResponse.FromString(
                        await MessageHandler.Client.SendMessageAndGetReplyAsync(Command.CheckForDowngradeNotify));
                if (response.StatusCode == StatusCode.Downgrade) TextBlockUpdateInfo.Text = "A downgrade is available";
            }
        }
        catch (Exception ex)
        {
    ...
    {
        if ((e.Key == Key.Enter) | (e.Key == Key.Space)) HyperlinkOpenConfigFile_PreviewMouseDown(this, null);
    }

    private void HyperlinkOpenLogFile_PreviewMouseDown(object sender, MouseButtonEventArgs e)
    {
        var filepath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
            "AutoDarkMode", "service.log");
        new Process
        {
            StartInfo = new ProcessStartInfo(filepath)
            {
    ...
    {
        if ((e.Key == Key.Enter) | (e.Key == Key.Space)) HyperlinkOpenLogFile_PreviewMouseDown(this, null);
    }
    ...
    {
        if ((e.Key == Key.Enter) | (e.Key == Key.Space)) HyperlinkOpenAppDataFolder_PreviewMouseDown(this, null);
    }
    ...
    private async void ToggleAutostart_Toggled(object sender, RoutedEventArgs e)
    {
        ...
        if (!(sender as ToggleSwitch).IsOn)
        {
            ...
            try
            {
                ...
                result = ApiResponse.FromString(
                    await MessageHandler.Client.SendMessageAndGetReplyAsync(Command.AddAutostart));
                await GetAutostartInfo(true, false);
                if (result.StatusCode != StatusCode.Ok)
                    throw new AddAutoStartException("Could not add Auto Dark Mode to autostart",
                        "AutoCheckBox_Checked");
            }
            catch (Exception ex)
            {
        ...
        {
            MsgBox confirm = new(
                "Auto Dark Mode will no longer start with Windows and switch your theme when you log in." +
                "\nDo you really want to do this? ", "Disable Autostart", "info", "yesno")
            {
                Owner = Window.GetWindow(this)
            };
            var dialogResult = confirm.ShowDialog() ?? false;
            if (!dialogResult)
            {
            ...
            try
            {
                ...
                builder.Save();
                result = ApiResponse.FromString(
                    await MessageHandler.Client.SendMessageAndGetReplyAsync(Command.RemoveAutostart));
                await GetAutostartInfo(true, false);
                (sender as ToggleSwitch).IsOn = false;
                if (result.StatusCode != StatusCode.Ok)
                    throw new AddAutoStartException("Could not remove Auto Dark Mode to autostart",
                        "AutoCheckBox_Checked");
            }
            catch (Exception ex)
            {
    ...
    private async void ValidateAutostart()
    {
        ...
        try
        {
            response = ApiResponse.FromString(
                await MessageHandler.Client.SendMessageAndGetReplyAsync($"{Command.ValidateAutostart} true", 2));
            if (response.StatusCode == StatusCode.Err) throw new AddAutoStartException();
